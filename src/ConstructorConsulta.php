...
class ConstructorConsulta
{
     ...
     * @throws \PIPE\Excepciones\ORM
     */
    public function obtenerClave($campo, $tipoRetorno = PIPE::ARREGLO)
    {
        if (in_array($tipoRetorno, [PIPE::CLASE, PIPE::SQL])) {
            Error::mostrar(Mensaje::$mensajes['RETORNO_NO_SOPORTADO']);
        }

        if ($tipoRetorno == PIPE::JSON) {
            $datos = $this->obtenerDatosClavePersonalizada(
                $this->_obtenerDatosConsultaSQL(PIPE::ARREGLO), $campo
            );

            return json_encode($datos);
        }

        return $this->obtenerDatosClavePersonalizada(
            $this->_obtenerDatosConsultaSQL($tipoRetorno), $campo
        );
    }
     ...
     * @throws \PIPE\Excepciones\ORM
     */
    public function extraer($campo, $tipoRetorno = PIPE::ARREGLO)
    {
        if (!in_array($tipoRetorno, [PIPE::ARREGLO, PIPE::JSON])) {
            Error::mostrar(Mensaje::$mensajes['RETORNO_NO_SOPORTADO']);
        }

        if ($tipoRetorno == PIPE::JSON) {
            $datos = $this->extraerClave(
                $this->_obtenerDatosConsultaSQL(PIPE::ARREGLO), $campo
            );
     ...
     * @return array|string
     * 
     * @throws \PIPE\Excepciones\ORM|\PIPE\Excepciones\SQL
     */
     ...
     * @param array|object $registro registro
     * 
     * @return array|object
     */
     ...
     *
     * @param string $tipoRetorno        tipoRetorno
     * @param bool   $traducirParametros traducirParametros
     * 
     * @return string
     */
    ...
    private function _obtenerParametrosActualizacion()
    {
        ...
        foreach ($valoresPivote as $clave => $valor) {
            if (is_numeric($valor) || is_string($valor) || is_null($valor)) {
     ...
     *
     * @param array|string $registros   registros
     * @param string       $tipoRetorno tipoRetorno
     * 
     * @return object|array|string|null
     */
